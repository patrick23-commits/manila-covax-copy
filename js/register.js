window.onload = async ()=>{
    ...
    file_name.onchange = async ()=> {
        ...
        if(file.length > 0) {
            ...
            profile.src = "img/loading-buffering.gif"
            setTimeout(()=> {
    ...
    function changehElement(element) {
        ...
        for(let i = 0; i < target.parentElement.children.length; i++) {
            try {
                if(target.parentElement.children[i].style.backgroundColor == "rgb(19, 134, 241)" && target.classList == "h1") {
